...
- Reused the exact tested distributions for both the GitHub and PyPI releases, with PyPI provenance attestations generated by the publishing action.
- Documented direct installation from PyPI with `python -m pip install biosigpy` while retaining GitHub release artifacts as a fallback.
